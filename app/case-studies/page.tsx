      ...
      <div className="grid md:grid-cols-2 gap-6">
        {PORTFOLIO_DATA.caseStudies.map((study, index) => (
          <Link key={index} href={`/case-studies/${study.slug}`} className="group block h-full">
            <div className="h-full flex flex-col rounded-3xl bg-zinc-900/50 border border-white/5 hover:bg-zinc-800/80 hover:border-cyan-500/30 hover:shadow-[0_8px_30px_rgba(6,182,212,0.1)] transition-all duration-500 hover:-translate-y-2 overflow-hidden">
              <div className="relative w-full aspect-[21/9] overflow-hidden">
                <Image src={study.cover} alt={study.title} fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
              </div>
